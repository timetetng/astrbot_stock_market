...
CUSTOM_CMAP = mcolors.LinearSegmentedColormap.from_list("market_cmap", list(zip(normalized_points, COLOR_HEX)))
NORM = mcolors.TwoSlopeNorm(vmin=-4.0, vcenter=0, vmax=4.0)


async def _get_stock_data_for_treemap(db_path: str) -> Optional[pd.DataFrame]:
    ...
    try:
        async with aiosqlite.connect(db_path) as db:
            ...
            for stock_id, name, current_price in stocks:
                ...
                ref_price = history_prices[PERIODS_FOR_30_MIN] if len(history_prices) > PERIODS_FOR_30_MIN else None
                change_percent = ((current_price - ref_price) / ref_price) * 100 if ref_price and ref_price > 0 else 0.0
                processed_data.append({"name": name, "price": current_price, "change_percent": change_percent})
...
def _generate_image(df: pd.DataFrame, output_path: Path):
    ...
    labels = [f"{row['name']}\n{row['change_percent']:+.2f}%\n${row['price']:.2f}" for _, row in df.iterrows()]    
    title_font_prop = FontProperties(fname=font_path, size=27)

    plt.style.use('dark_background')
    fig, ax = plt.subplots(1, figsize=(16, 9), dpi=200)
    
    squarify.plot(sizes=sizes, color=bg_colors, ax=ax, alpha=0.9, label=None, edgecolor="black", linewidth=1.5)
    ...
    plt.title("虚拟股票市场 - 大盘云图 (30分钟)", fontproperties=title_font_prop, color='white', pad=20)
    plt.axis('off')
    plt.tight_layout()

    try:
        plt.savefig(output_path, bbox_inches='tight', pad_inches=0.1, facecolor=fig.get_facecolor(), edgecolor='none')
    finally:
        plt.close(fig)
        plt.rcParams.update(plt.rcParamsDefault)


async def create_market_treemap(db_path: str, output_dir: str) -> Optional[str]:
    """生成大盘云图的主函数。"""
